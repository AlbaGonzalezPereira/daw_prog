...
public abstract class IndexedDatabase<K,R> implements Database<R> {
    ...
    //modificamos para poder usar o index
     ...
     *      Si o dato non está borrado e cumple o criterio: 
    ...
    public R[] delete(Criteria<R> c) throws DatabaseException {
        ...
        try (RandomAccessFile ras=new RandomAccessFile(filename,"rws")) {
            ...
            data=read(ras);
            while(data!=null) {
                if (!isDeleted(data) && c.accept(data)) {
                    deleted.add(data);
     ...
     * - Lemos datos
     * - Mentres teñamos datos
     *      Si o dato non está borrado e cumple o criterio o gardamos nun arraylist
     *      Lemos datos
     *  - Transformamos o arraylist nun Array e o retornamos
     */
    /*@Override
    public R[] select(Criteria<R> c) throws DatabaseException {
        ...
        try (RandomAccessFile ras=new RandomAccessFile(filename,"rws")) {
            data=read(ras);
            while(data!=null) {
                if (!isDeleted(data) && c.accept(data)) selected.add(data);
                data=read(ras);
            }
        } catch (Exception ex) {
    ...
    public void pack() throws DatabaseException {
        String temp="C:\\Users\\alba_\\AppData\\Local\\Temp"+filename;
        R r;
    ...
    //insertamos o método get para poder obter o index do ficheiro
    public Index<K> getIndex() {
        return index;
    }
    
    /**
     * Método que devolve a liña da chave do ficheiro correspondente. Posiciónase no
     * no inicio e vai lendo liñas mentres non chega ao final do ficheiro
     * @param key - chave do csv
     * @return - Devolve o número de liña ou 0 se non o encontrou.
     */
    public long getLineForKey(K key){
        try (RandomAccessFile ras=new RandomAccessFile(filename,"rws")) {
           String cadena;
           String[] cadenas;
           long indice=1;
           ras.seek(0);//Para posicionar o punteiro o principio do ficheiro
           while(true){
               cadena=ras.readLine();
               cadenas=cadena.split(";");
               if(cadenas[0].equals(key)){
                   return indice;
               }
               indice++;    
           } 
            ...
            Logger.getLogger(IndexedDatabase.class.getName()).log(Level.SEVERE, null, ex);
        } catch (IOException ex) {
            Logger.getLogger(IndexedDatabase.class.getName()).log(Level.SEVERE, null, ex);
        }
        return 0;
